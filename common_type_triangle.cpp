...
void TGrid_DC_Full_Triangle::fill_TGrid_Triangle(std::string filename)
{
    max_diag = 0.;
    int t = 0, sum = 0;
    int kobj = 0, kmod = 0, sign = 0, nfm = 0, i11 = 0, i22 = 0;
    double diag1[3], diag2[3], diag3[3], len1, len2, len3;
    ...
    for (int i = 0; i < kobj; i++) {
        ...
        for (int j = 0; j < kmod; j++) {
            ...
            for (int k = 0; k < nfm; k++) {
                ...
                for (int g = 0; g < 3; g++)
                {
                    diag1[g] = root[sum][1][g] - root[sum][0][g];
                    diag2[g] = root[sum][2][g] - root[sum][1][g];
			        diag3[g] = root[sum][0][g] - root[sum][2][g];
                }
                len1 = vec_length(diag1);
                len2 = vec_length(diag2);
		        len3 = vec_length(diag3);
                if (len1 > max_diag)
                {
                    max_diag = len1;
                }
    ...
    for (int i = 0; i < num_frm; i++) {
        for (int j = 0; j < 3; j++) {
            rkt[i][j] = (root[i][0][j] + root[i][1][j] +
                    root[i][2][j]) / 3;
                for (int k = 0; k < 3; k++) {
                ab[k] = root[i][1][k] - root[i][0][k];
                ac[k] = root[i][2][k] - root[i][0][k];
            }
            vec_prod(ab, ac, norm[i]);
...
//------------------------------------------Constructor-------------------------------------------------------
TGrid_DC_Full_Triangle::TGrid_DC_Full_Triangle(std::string filename)//сквозная сетка на все объекты и все модули вместе
{
